...
#define INPUT_BUFFER_SIZE (500 * 8192)
...
int main(int argc, char *argv[])
{
	...
	int error;
	char buf[819200];
	struct mp3play_ctx ctx;


	wrapper_init(&ctx);
	count = read(0, buf, 819200);

	fprintf(stderr, "delta data %d\n", count);
	{
	char *p = buf;
	while (count > 0) {
		int ret = wrapper_feed(&ctx, p, count);
	...
	do {
		count = wrapper_frame(&ctx, buf, sizeof(buf));
		while (count > 0) {
			write(1, buf, count);
