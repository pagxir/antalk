...
void subdivide(int *scalefac_band_long, gr_info *gi);
int count1_bitcount(int ix[samp_per_frame2], gr_info *gi);
void calc_runlen(struct shine_ctx *shine, int ix[samp_per_frame2], gr_info *gi);
int quantize(struct shine_ctx *shine, int ix[samp_per_frame2], int stepsize);
 ...
 */
static int inner_loop(struct shine_ctx *shine, int ix[samp_per_frame2],int max_bits,gr_info *gi)
{
	 ...
	 */
	while (step) {
		if (!quantize(shine, ix, stepsize))
			bits = 0x7fffffff;     /* outside (our) table range, early fail exit */
		else {
			calc_runlen(shine, ix, gi);              /* rzero,count1,big_values */
			bits = count1_bitcount(ix, gi);   /* count1_table selection */
			subdivide(shine->scalefac_band_long, gi);                   /* bigvalues sfb division */
	 ...
	 */
	while(bits > max_bits) {
		quantize(shine, ix, ++stepsize);         /* update ix */
		calc_runlen(shine, ix, gi);              /* rzero,count1,big_values */
		bits = count1_bitcount(ix, gi);   /* count1_table selection */
		subdivide(shine->scalefac_band_long, gi);                   /* bigvalues sfb division */
		bigv_tab_select(ix, gi);          /* codebook selection */
		...
		int mean_bits)
{
	...
	reservoir = shine->main_data_begin << 3;  /* calculate reservoir at the frame start */
	side_info->main_data_begin = shine->main_data_begin; /* set next frames back pointer */

	for (gr = 0; gr < shine->granules; gr++)
		for (ch = shine->channels; ch--; ) {
			/* setup pointers */
			ix = l3_enc[gr][ch];
			shine->xr = mdct_freq_org[gr][ch];
			gi = &side_info->gr[gr].ch[ch].tt;

			/* calculate absolute and maximum */
			for (i = shine->cutoff, shine->xrmax = 0; i--; )
				if (shine->xrmax < (shine->xrabs[i] = abs(shine->xr[i])))
					shine->xrmax = shine->xrabs[i];
			...
			if (shine->xrmax)
				gi->part2_3_length = inner_loop(shine, ix, max_bits, gi);
			...
			for ( i = 0; i < shine->cutoff; i++ )
				if (shine->xr[i] < 0)
					ix[i] *= -1;
		}

	/* Sort out reservoir at frame end, limit the size and stuff the excess */
	resv_max = DEC_BUFF_LIMIT - shine->bits_per_frame;
	if (resv_max > shine->resv_limit)
		resv_max = shine->resv_limit;
	else if (resv_max < 0)
		resv_max = 0;

	shine->main_data_begin = (reservoir < resv_max) ? (reservoir >> 3) : (resv_max >> 3);
	extra_bits = reservoir - (shine->main_data_begin << 3);
...
int quantize(struct shine_ctx *shine, int ix[samp_per_frame2], int stepsize )
{
	 ...
	 */
	if (mulr(shine->xrmax, scale) > 9999)
	 ...
	 */
	for (i = 0; i < shine->cutoff; i++)
		ix[i] = pow3_4[mulr(shine->xrabs[i], scale)];
...
void subdivide(int *scalefac_band_long, gr_info *gi)
{
	...
	if ( !gi->big_values) { /* no big_values region */
		gi->region0_count = 0;
		gi->region1_count = 0;
	} else {
