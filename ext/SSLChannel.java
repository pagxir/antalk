 ...
 * @author Nuno Santos
 */
public class SSLChannel implements ReadableByteChannel {
	private final static Logger log = Logger.getLogger("handlers");

	private final SSLSession session;
	private final SSLEngine engine;
	private final SlotWait readIn = new SlotWait() {
		public void invoke() {
...
*/
	...
	private final SlotWait writeOut = new SlotWait() {
		public void run() {
...
*/
	...
	public SSLChannel(SlotChannel slot, SocketChannel sc, SSLEngine engine) throws Exception {
	...
	private SlotSlot reader = new SlotSlot();
	private SlotSlot writer = new SlotSlot();
	public void wantIn(SlotWait wait) {
		if (initialHandshake) {
			reader.record(wait);
			return;
		}
	...
	public void wantOut(SlotWait wait) {
		if (initialHandshake) {
			writer.record(wait);
			return;
		}
	...
	void wantInInternal(SlotWait wait) {
		assert(wait == readIn);
		slot.wantIn(wait);
	}

	void wantOutInternal(SlotWait wait) {
		slot.wantOut(wait);
	}
	...
	public boolean needReadMore() {
		if (initialHandshake)
			return engine.getHandshakeStatus() == SSLEngineResult.HandshakeStatus.NEED_UNWRAP;
		return false;
	}
